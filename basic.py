...
for col in categorical_columns:
    ...
    plt.xlabel(col)
    plt.ylabel("Mean L-DOPA Concentration")
    plt.title(f"L-DOPA Concentration vs {col}")
...
for f1, f2 in feature_pairs:
    ...
    plt.title(f"Conditional Mean LDOPA: {f1} × {f2}")
    plt.ylabel(f1)
    plt.xlabel(f2)
...
for factor in factors:
    ...
    plt.xticks(rotation=45)
    plt.xlabel(factor)
    plt.ylabel("Predicted L-DOPA Concentration")
    plt.title(f"Main Effect of {factor} on Predicted L-DOPA")
...
for f1, f2 in pairwise_factors:
    ...
    sns.heatmap(
        ...
        cmap="viridis",
        annot=False,
        cbar_kws={"label": "Predicted L-DOPA"}
    )
    plt.xlabel(f2)
    plt.ylabel(f1)
    plt.title(f"Response Surface Heatmap: {f1} × {f2}")
...
# ------------------------------------------------------------------
# 2️⃣ Robust Score vs Mean Prediction (STATIC)
# ------------------------------------------------------------------
plt.figure(figsize=(6, 5))
plt.scatter(
    opt_df["Mean_Predicted_LDOPA"],
...
plt.xlabel("Mean Predicted L-DOPA")
plt.ylabel("Robust Score (μ − λσ)")
plt.title("Robust Optimization Trade-off")
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "RobustScore_vs_Mean.png"), dpi=300)
plt.close()

# ------------------------------------------------------------------
# 3️⃣ Top-10 Robust Conditions with Confidence Intervals (STATIC)
# ------------------------------------------------------------------
top10 = robust_ranking.head(10)

plt.figure(figsize=(8, 5))
plt.errorbar(
    range(10),
    top10["Mean_Predicted_LDOPA"],
    yerr=[
        top10["Mean_Predicted_LDOPA"] - top10["CI_05"],
        top10["CI_95"] - top10["Mean_Predicted_LDOPA"]
    ],
    fmt="o",
    capsize=5
)
plt.xticks(range(10), range(1, 11))
plt.xlabel("Robust Rank")
plt.ylabel("Predicted L-DOPA")
plt.title("Top-10 Robust Conditions (90% CI)")
plt.tight_layout()
plt.savefig(os.path.join(FIG_DIR, "Top10_Robust_CI.png"), dpi=300)
plt.close()

# ------------------------------------------------------------------
# 4️⃣ Distribution of Prediction Uncertainty (STATIC)
# ------------------------------------------------------------------
plt.figure(figsize=(6, 4))
plt.hist(
    opt_df["Std_Predicted_LDOPA"],
